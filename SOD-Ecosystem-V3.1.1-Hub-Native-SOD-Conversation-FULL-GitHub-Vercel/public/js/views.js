...
function hubView(){const s=state();const groups=[
  ...
  mount(){
    ...
    const openConversation=()=>{conversation.hidden=false;conversation.setAttribute('aria-hidden','false');requestAnimationFrame(()=>conversation.classList.add('open'));document.documentElement.classList.add('sod-conversation-open');scene?.pause();setTimeout(()=>conversation.querySelector('[data-chat-input]')?.focus(),180)};
    const closeConversation=()=>{conversation.classList.remove('open');conversation.setAttribute('aria-hidden','true');document.documentElement.classList.remove('sod-conversation-open');setTimeout(()=>{conversation.hidden=true;scene?.resume();canvas.focus()},260)};
